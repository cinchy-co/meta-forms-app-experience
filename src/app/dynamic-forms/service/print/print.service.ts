...
export class PrintService {
  ...
  constructor(private appStateService: AppStateService, private spinner: NgxSpinnerService, @Inject(LOCALE_ID) public locale: string,
  ...
  getFieldColumns(field, overrideValue?) {
    return [{
      width: 130,
      text: `${field.label}: `,
      style: 'fieldLabel'
  ...
  getLinkFieldLabelAndValue(field) {
    const dropdownSet = field.dropdownDataset?.options;
    if (dropdownSet && field.cinchyColumn.isMultiple) {
      let selectedOptions;
      const fieldValues = field.value instanceof Array ? field.value : field.value ? field.value.split(',') : '';
      if (field.value) {
  ...
  getChildFormTable(form) {
    ...
    let childFormRowValues = form.sections[0]?.childFormRowValues;
    ...
    if (childFormRowValues?.length) {
      ...
      childFormRowValues.forEach(field => {

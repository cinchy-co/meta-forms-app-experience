...
export class SaveSuccessComponent implements OnInit {
  ...
  async getSeatingInformation() {
    try {
      const seatResp = await this._cinchyService.executeQuery('Human Resources', 'Get Assigned Seat', null).toPromise();
      ...
      await this._cinchyService.executeQuery('Human Resources', 'Set Assigned Seat',
        {'@seatNumber': this.assignedSeat}).toPromise();
      await this._cinchyService.executeQuery('Human Resources', 'Set Restricted Seats',
